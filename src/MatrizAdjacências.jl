...
export MatrizAdjacência, cópia, vazio, nulo, matriz_adj, lista_adj,
...
"""
cópia(g::MatrizAdjacência)::MatrizAdjacência = MatrizAdjacência(g.n,g.m, copy(g.mat))
...
"""
vizinhos(g::MatrizAdjacência, v::Integer) = [ idx for (idx,v) in enumerate(g.mat[:,v]) if v != 0 ]
...
remove_aresta!(g::MatrizAdjacência, (u,v)) = remove_aresta!(g::MatrizAdjacência, u, v)
